package com.xerprojects.xerj.commandstack.exceptions;

import com.xerprojects.xerj.commandstack.Command;

public class DuplicateCommandHandlerRegistrationException extends RuntimeException {

	private static final long serialVersionUID = 1457417443813122941L;
	private static final String DEFAULT_EXCEPTION_MESSAGE = "Attempting to register duplicate command handlers for %s.";
	
	private final Class<? extends Command> commandType;
	
	public DuplicateCommandHandlerRegistrationException(Class<? extends Command> commandType, String message, Throwable cause) {
		super(message, cause);
		this.commandType = commandType;
	}
	
	public DuplicateCommandHandlerRegistrationException(Class<? extends Command> commandType, String message) {
		this(commandType, buildExceptionMessage(commandType, message), null);
	}
	
	public DuplicateCommandHandlerRegistrationException(Class<? extends Command> commandType) {
		this(commandType, null);
	}

	public Class<? extends Command> getCommandType() {
		return commandType;
	}
	
	private static final String buildExceptionMessage(Class<? extends Command> commandType, String message) {

		if (message == null || message.isBlank()) {
			message = String.format(DEFAULT_EXCEPTION_MESSAGE, commandType);
		}
		
		return message;
	}
}
